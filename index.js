...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} corno(a) 🐂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Carregando... Caso não obtenha, tente novamente! ⏳',
				success: '✔️ Sucesso ✔️',
				error: {
					stick: '❌ Ocorreu um erro ao converter a imagem ou gif em figurinha ⚠️',
					Iv: '❌ Link tidak valid ❌'
				},
				only: {
					group: '❌ Este comando só pode ser usado em grupos! ❌',
					ownerG: '❌ Este comando só pode ser usado pelo grupo proprietário! ❌',
					ownerB: '❌ Este comando só pode ser usado pelo Herberth lindo123! ❌',
					admin: '❌ Este comando só pode ser usado pelos administradores do grupo, seu membro comum! 🖕❌',
					Badmin: '❌ Este comando só pode ser usado quando o bot se torna  administrador! ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5511996237647@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'sticker':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie fotos com legendas ${prefix}sticker para criar a figurinha!`)
					}
					break
					case 'gtts':
					...
					ranm = getRandom('.mp3')
					dtt.length > 600
					? reply('❌ Texto muito longo, por Favor, diminua o texto! ❌')
				...
				case '+18':
					...
					buffer = await getBuffer(`https://i.imgur.com/F4EfPr2.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'O adm proibiu porno no gp🙄'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/XK6rIMy.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Herberth\n*YT:* youtube.com/c/herberthduarte\n*WPP:* wa.me/+5511996237647\n*INSTA:* instagram.com/herberthsfc\n\n\nfrio e calculista'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'mia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/gJHsM6e.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Tem que se fod*r e acabou, taok?!'})
					break
				case 'lofi':
					memein = await kagApi.memeindo()
				...
				case 'yt':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/cAVzUDz.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do herberth:*\n\n https://youtube.com/c/herberthduarte'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
				...
				case 'prefixo':
					...
					prefix = args[0]
					reply(`O prefixo foi alterado com sucesso para ✔️ : ${prefix}`)
					break
				case 'loli':
                ...
                case 'marcar2':
                 ...
                 case 'marcar3':
					...
					break
				case 'limpar':
					if (!isOwner) return reply('❌ Este comando só pode ser usado pelo Herberth ❌')
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('Excluido todos os chats com sucesso✔️ :)')
					break
				case 'ts':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ Transmissão de HDBot.exe ]\n\n${body.slice(4)}`})
						}
						reply('Transmissão enviada com sucesso✔️')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ Transmissão de HDBot.exe ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão enviada com sucesso✔️')
					}
					break
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`NOVO(A) ADM! @${mentioned[0].split('@')[0]} Deixou de ser membro(a) comum e passou a ser Administrador(a) do Grupo👮✔️`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`MEMBRO(A) COMUM! @${mentioned[0].split('@')[0]} Foi rebaixado(a) de ADM para membro(a) comum!✔️`, mentioned, true)
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque o número que voce deseja remover!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'SE FODEU! Removido com sucesso✔️ :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`SE FODEU! Removido com sucesso! : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'ademirs':
